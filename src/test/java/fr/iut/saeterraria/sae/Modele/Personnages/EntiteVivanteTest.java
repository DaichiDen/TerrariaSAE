...
class EntiteVivanteTest {
   ...
   private Jeu jeu;
    ...
    void peutEtreAtteint() {
        ...
        assertFalse(joueur.peutEtreAtteint(576/32,736/32,2)); // Test 1 , on place le joueur à 3 blocs de différence du bloc souhaité, le teste doit retourner false car la range est de 2

        joueur.setX(512);
        joueur.setY(736);

        assertTrue(joueur.peutEtreAtteint(576/32,736/32,2)); // Test 2, on place le joueur à 2 blocs de différence du bloc souhaité, le test doit retourner true car on mets une range de 2 et il est à 2 blocs de différence
    }
}